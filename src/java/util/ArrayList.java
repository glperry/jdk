        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    /**
     * ArrayList无参构造,初始化的是一个空数组,elementData数组长度为0
    ...
    public ArrayList(Collection<? extends E> c) {
        ...
        if ((size = elementData.length) != 0) {
            ...
            //如果集合元素不是Object[]类型,就会创建新的Object[]数组,并将elementData赋值到其中
    ...
    /**
     * 无参ArrayList,首次扩容时,minCapacity=size+1=1
     * elementData.length=10,size=10,minCapacity=size+1=11
    ...
    private Object[] grow(int minCapacity) {
        ...
        if (oldCapacity > 0 || elementData != DEFAULTCAPACITY_EMPTY_ELEMENTDATA) {
            //新建一个数组,将原来的数组数据复制到新数组中
            //新数组的长度为10+5{max(minGrowth=1,prefGrowth=5)+oldLength}
            //新数组的长度为15+7{15+max(minGrowth=1,prefGrowth=15>>1=7)}
            //oldCapacity=10,minGrowth=1,prefGrowth=5
            int newCapacity = ArraysSupport.newLength(oldCapacity,
                    //11-10=1
                    minCapacity - oldCapacity, /* minimum growth */
                    //10>>1=5,15=8+4+2+1=2^3+2^2+2^1+2^0=1111>>1
                    oldCapacity >> 1           /* preferred growth */);
            return elementData = Arrays.copyOf(elementData, newCapacity);
        } else {
            //无参ArrayList首次添加元素,elementData数组扩容到10
    ...
    /**
     * 无参构造初始化ArrayList时,其内数组的长度为0,size也为0
     * 首次添加元素时,会执行grow(),扩容到10
     * 如果ArrayList中元素到达了10,再添加第11个的时候,
     * 此时elementData.length=10,size=10,再次扩容
    ...
    //ArrayList的移除,是内部新建一个数组,将除了被移除外的元素都复制到新数组中
    ...
    private void fastRemove(Object[] es, int i) {
        ...
        final int newSize;
        if ((newSize = size - 1) > i)
            //es=["gl","glperry"] ,i=0,如果不是最后一个元素,es的元素前挪1位
     ...
     * @throws NullPointerException if the specified collection is null
     */
    //批量插入的集合数量多,grow(s+numNew)至少扩容到能装下新增的,但是
    //如果s+numNew扩容太小<oldCapacity,则选择oldCapacity>>1,扩容至1.5倍
