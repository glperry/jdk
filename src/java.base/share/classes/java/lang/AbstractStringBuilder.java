...
/**
 * StringBuilder相当于String的操作类,可以方便进行Spring的操作,它的优点为:
 * 如果是String的拼接,由于String是final修饰,多次拼接,内存中new多出个String.占用内存
 *
 * 而如果使用StringBuilder(下文用builder),builder执行拼接,是将要拼接的字符串分解为字符数组,
 * 存储字符数组ASCII码表中对应的数值.builder中的重要属性byte[] value就是存储 这些数值,
 * int count是保存存储的字符的个数.不需要new String对象.
 * 所以builder.append("a").append("b").append("c").toString只会在内存中生成一个String对象,极大减少内存的占用.
 */
